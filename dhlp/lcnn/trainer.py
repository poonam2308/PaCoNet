...
class Trainer(object):
    def __init__(self, device, model, optimizer, train_loader, val_loader, out):
        ...
        os.makedirs(self.board_out, exist_ok=True)
    ...
    def train_epoch(self):
        ...
        for batch_idx, (image, meta, target) in enumerate(self.train_loader):
            ...
            loss.backward()
            self.optim.step()
            self.avg_metrics = self.metrics if self.avg_metrics is None else self.avg_metrics * 0.9 + self.metrics * 0.1
...
def imshow(im):
    plt.close()  # Ensure any existing plots are cleared
    plt.tight_layout()
    img = plt.imshow(im, cmap="jet")  # Associate image with color map
    plt.colorbar(img, fraction=0.046)  # Use the mappable returned by plt.imshow
    plt.xlim([0, im.shape[1]])  # Note: shape[1] corresponds to width
    plt.ylim([im.shape[0], 0])  # Note: shape[0] corresponds to height

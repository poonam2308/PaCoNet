...
def main():
    ...
    for batch in ["sdist_train", "sdist_valid"]:
        ...
        def handle(data):
            ...
            lines0 = lines.copy()
            lines1 = lines.copy()
            lines1[:, :, 0] = im.shape[1] - lines1[:, :, 0]  # horizontal flip
            lines2 = lines.copy()
            lines2[:, :, 1] = im.shape[0] - lines2[:, :, 1]  # vertical flip
            lines3 = lines.copy()
            lines3[:, :, 0] = im.shape[1] - lines3[:, :, 0]    #combined flip
            lines3[:, :, 1] = im.shape[0] - lines3[:, :, 1]    #combined flip

            path = os.path.join(data_output, batch, prefix)
            save_heatmap(f"{path}_0", im[::, ::], lines0)
            if batch != "sdist_valid":

...
from src.dhlp.lcnn.config import M
...
class WireframeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        iname = re.sub(r'(_a0|_a1)(?=_label\.npz$)', '', self.filelist[idx][:-10]) + ".png"
        # iname = self.filelist[idx][:-10].replace("_a0", "").replace("_a1", "") + ".png"
        image = io.imread(iname).astype(float)[:, :, :3]
        if "a1" in self.filelist[idx]:
            image = image[:, ::-1, :]

        # Handle flipping based on suffix
        # iname = self.filelist[idx][:-10] + ".png"
        # image = io.imread(iname).astype(float)[:, :, :3]
        # if "_1" in self.filelist[idx]:  # Combined flip version
        #     image = image[::-1, ::-1, :]  # Apply combined horizontal + vertical flip

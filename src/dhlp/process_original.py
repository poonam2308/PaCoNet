    ...
    for batch_idx, (image, meta, target) in enumerate(loader):
        with torch.no_grad():
            ...
            for i in range(len(image)):
                # Extract corresponding image filename from the dataset's filelist
                image_filename = osp.splitext(osp.basename(loader.dataset.filelist[batch_idx * M.batch_size + i]))[0]
                np.savez(
                    osp.join(output_dir, f"{image_filename}.npz"),

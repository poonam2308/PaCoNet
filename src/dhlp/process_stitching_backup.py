...
def stitch_parallel_coordinates_keep_all(
    file_paths,
    column_map,
    category_id=1,
    threshold=5.0,
    ...
    """
    Branching / retain-all stitching.

    For each step i -> i+1:
      - For every line in next crop (y1_next, y2_next), find nearest previous path endpoint (prev crop's y2).
      - Create a new path as a copy of that best previous path, then set crop{i+2}=y2_next.
      - This allows many-to-one (branching) so ALL next-crop lines are retained.
      - If no predecessor within threshold (or no existing paths), optionally start a new "born" path.

    Output columns: crop1, crop2, ..., cropN + cat
      - crop1 stores y1 of first crop line
      - crop2 stores y2 of first crop line
      - crop(k+1) stores y2 of crop k line
    """
    dfs = []
    for fp, (y1_col, y2_col) in zip(file_paths, column_map):
        ...
        # keep only the two cols we need, rename to y1/y2
    ...
    # Seed paths from crop1: crop1=y1, crop2=y2
    paths = [{"crop1": float(r["y1"]), "crop2": float(r["y2"])} for _, r in dfs[0].iterrows()]

    # Iterate over subsequent crops
    for i in range(1, len(dfs)):
        candidates = dfs[i]
        next_paths = []

        prev_key = f"crop{i+1}"   # previous crop's endpoint (y2) stored here
        next_key = f"crop{i+2}"   # we will store next crop's y2 here

        # If there are no existing paths, everything in this crop becomes a "born" path
        if len(paths) == 0:
            if allow_birth:
                for _, cand in candidates.iterrows():
                    p = {f"crop{k}": fill_value for k in range(1, i+2)}  # crop1..crop{i+1}
                    p[next_key] = float(cand["y2"])
                    next_paths.append(p)
            paths = next_paths
            continue

        endpoints = np.array([p.get(prev_key, np.nan) for p in paths], dtype=float)

        # IMPORTANT: retain-all-next-lines
        for _, cand in candidates.iterrows():
            y1_next = float(cand["y1"])
            y2_next = float(cand["y2"])

            # find nearest previous endpoint to this next start
            diffs = np.abs(endpoints - y1_next)
            j = int(np.nanargmin(diffs)) if np.isfinite(diffs).any() else None

            if j is not None and np.isfinite(diffs[j]) and diffs[j] <= threshold:
                # attach (branch) to nearest predecessor
                p2 = paths[j].copy()
                p2[next_key] = y2_next
                next_paths.append(p2)
            else:
                # no good predecessor: keep it anyway (birth)
                if allow_birth:
                    p2 = {f"crop{k}": fill_value for k in range(1, i+2)}
                    p2[next_key] = y2_next
                    next_paths.append(p2)
                # else: drop it

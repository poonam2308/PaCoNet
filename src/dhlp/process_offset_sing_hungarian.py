...
def hungarian_offsets(pred_points, gt_junctions):
    """
    pred_points   : (Np, 2) array of (y, x)
    gt_junctions  : (Ng, 3) or (Ng, 2) array, first two columns are (y, x)
    returns       : (Nk,) array of distances for matched pairs,
                    where Nk = min(Np, Ng)
    """
    if pred_points is None or len(pred_points) == 0:
        return np.array([])

    if gt_junctions is None or len(gt_junctions) == 0:
        return np.array([])

    gt_xy = gt_junctions[:, :2]  # (Ng, 2) — ignore the 3rd channel if present

    # Cost matrix: pairwise Euclidean distances between pred and GT
    # shape: (Np, Ng)
    diff = pred_points[:, None, :] - gt_xy[None, :, :]
    dists = np.linalg.norm(diff, axis=-1)

    # Hungarian matching
    row_ind, col_ind = linear_sum_assignment(dists)

    # Distances for the optimal 1-1 matches
...
def main():
    ...
    output_file = "offset_results_lines_cls_27.txt"
        ...
        for batch_idx, (image, meta, target) in enumerate(loader):
            with torch.no_grad():
                ...
                for i in range(len(image)):
                    ...
                    line_endpoints = H["lines"][i].cpu().numpy()
                    # Get ground truth junctions
                    ground_truth_junctions = meta[i]["junc"].cpu().numpy()
                    ...
                    print("---- DEBUGing ----")
                    print("GT junc shape:", ground_truth_junctions.shape)
                    print("GT junc min/max:", ground_truth_junctions.min(axis=0),
                          ground_truth_junctions.max(axis=0))
                    print("Start pts shape:", start_points.shape)
                    print("Start min/max:", start_points.min(axis=0), start_points.max(axis=0))
                    print("End   min/max:", end_points.min(axis=0), end_points.max(axis=0))



                    start_offsets = hungarian_offsets(start_points, ground_truth_junctions)
                    end_offsets = hungarian_offsets(end_points, ground_truth_junctions)

...
def base_stem(stem: str) -> str:
    """
    From a stem like 'image_1_crop_1_x' or 'image_42_crop_A_anything',
    return the base 'image_1_crop_1' or 'image_42_crop_A'.
    If it doesn't match the expected pattern, return the original stem.
    """
    # Strict (digits only): r'^(image_\d+_crop_\d+)'
    m = re.match(r'^(image_[^_]+_crop_[^_]+)', stem)
...
def main():
    ...
    for batch in ["all_test", "valid"]:
        ...
        # ---- NEW: group entries by base filename, aggregate lines ----
        groups = {}  # key: (base_stem, ext) -> dict(filename=<base+ext>, lines=[...])
        for entry in dataset:
            fname = entry["filename"]
            stem, ext = os.path.splitext(fname)
            bstem = base_stem(stem)
            key = (bstem, ext)

            if key not in groups:
                groups[key] = {"filename": bstem + ext, "lines": []}

            # Append this entry's lines to the shared pool
            groups[key]["lines"].extend(entry.get("lines", []))

        grouped_dataset = list(groups.values())
        ...
        with Pool(processes=8) as pool:  # Adjust number of processes if needed
            ...
            pool.starmap(handle, [(data, data_root, data_output, batch) for data in grouped_dataset])
    end_time = time.time()

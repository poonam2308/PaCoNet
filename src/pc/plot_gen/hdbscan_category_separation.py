...
class HDBSCANCategorySeparator:
    """
    HDBSCAN-based color category separation (white background), similar to ClusteringCategorySeparator.

    Workflow per base image:
      1) Group all its _crop_# images.
      2) Pick the "best" crop (most clusters by HDBSCAN on downsampled hue).
    ...
    Notes:
      ...
      - HDBSCAN parameters:
          * eps is **ignored** (kept for API compatibility with DBSCAN version).
          * min_samples is interpreted as HDBSCAN's `min_cluster_size`.
    ...
    def _cluster_hues(self, img_bgr, resize_factor, min_cluster_size,
                      min_samples_hdb=None, sat_thresh=50, val_thresh=50):
        """
        Run HDBSCAN on downsampled hue channel (only moderately saturated/bright pixels).
        Return: dict cluster_id -> (min_hue, max_hue, center_hue), and list of kept cluster ids.

        Parameters:
          min_cluster_size : int
              Interprets the public `min_samples` parameter from process_* as HDBSCAN's min_cluster_size.
          min_samples_hdb : int or None
              Optional HDBSCAN `min_samples`. If None, HDBSCAN will default it.
        """
        ...
        # HDBSCAN clustering
        if min_samples_hdb is not None:
            clusterer = HDBSCAN(min_cluster_size=min_cluster_size, min_samples=min_samples_hdb)
        else:
            clusterer = HDBSCAN(min_cluster_size=min_cluster_size)

        lab = clusterer.fit_predict(hue_vals)
    ...
    def _cluster_hues_fullres(self, img_bgr, min_cluster_size,
                              min_samples_hdb=None, sat_thresh=50, val_thresh=50):
        """
        Run HDBSCAN on full-resolution hue channel (only moderately saturated/bright pixels).
        Return: dict cluster_id -> (min_hue, max_hue, center_hue), and list of kept cluster ids.
        """
    ...
    def process_single_image(
        ...
        output_dir=".",
        resize_factor=0.30,
        eps=None,              # kept for API compatibility, not used
        min_samples=200,       # interpreted as HDBSCAN's min_cluster_size
        sat_thresh=50,
        val_thresh=50,
        min_samples_hdb=None   # optional true HDBSCAN min_samples
    ):
        """
        Discover hue clusters **on this image** using HDBSCAN and apply them to this image only.
        White background cutouts. Returns (output_data, color_data).

        Parameters:
          eps : ignored (kept for compatibility).
          min_samples : int
              Used as HDBSCAN's `min_cluster_size`.
          min_samples_hdb : int or None
              Optional HDBSCAN `min_samples`. If None, HDBSCAN uses its default.
        """
        os.makedirs(output_dir, exist_ok=True)
        ...
        ranges, kept = self._cluster_hues(
            ...
            min_samples_hdb=min_samples_hdb,
    ...
    def process_batch(
        ...
        json_dir=None,
        output_dir=".",
        eps=None,                 # kept for compatibility, not used
        min_samples=200,          # interpreted as HDBSCAN min_cluster_size
        sat_thresh=50,
        val_thresh=50,
        width_threshold=200,
        ...
        prefer_gt_lines=True,
        min_samples_hdb=None      # optional HDBSCAN min_samples
    ):
        """
        Batch mode (HDBSCAN version):
          - Group files by base (strip `_crop_#`).
          - For each base, choose the crop with most clusters as the "best" crop.
          - Compute hue ranges on the best crop (downsampled hue + HDBSCAN).
          - Apply those ranges to all crops in that base group.
          - Save white-bg cutouts and three JSONs: all_data.json, all_colors.json, cluster_vs_gt*.json
        """
        os.makedirs(output_dir, exist_ok=True)
        comparisons = []
        ...
        print(f"[DONE] Saved HDBSCAN batch results in: {output_dir}")

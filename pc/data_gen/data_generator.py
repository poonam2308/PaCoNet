...
def generate_synthetic_datasets_from_distributions(
    directory_path,
    k,
    axes_distribution,
    ...
    for i in range(k):
        num_columns = np.random.choice(axes_distribution.index, p=axes_distribution.values)
        num_categories = np.random.choice(categories_distribution.index, p=categories_distribution.values)
        num_rows = np.random.choice(rows_distribution.index, p=rows_distribution.values)
        ...
        for category in categories:
            ...
            category_mean = np.random.rand(num_columns) * 10
            category_data = np.random.multivariate_normal(category_mean, cov, mask.sum())
            for j in range(num_columns):
                category_data[:, j] *= scaling_factors[j % len(scaling_factors)]
            df.loc[mask, df.columns[:-1]] = category_data
    ...
    print(f"[✓] Generated {k} datasets using real-world distributions.")
